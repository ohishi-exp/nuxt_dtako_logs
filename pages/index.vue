  ...
  if (gmap.value != undefined && index2 != -1 && index2 != undefined) {
    ...
    columns.forEach((v) => {
      ...
      if (v.key) {
        switch (v.key) {
          case "DataDateTime":
            sst += v.label + ":" + new Date(row[v.key].value).toLocaleTimeString("ja-jp", { year: "2-digit", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", "second": "2-digit" }) + "<br>"
            break
          case "AllState":
            sst += v.label + ":" + row[v.key].value
              .replace("Drive", "運転")
              .replace("Break", "休憩")
              .replace("Rest", "休息")
              .replace("End", "終了") + "<br>"
